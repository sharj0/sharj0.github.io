...
def main(settings_path):
    ...
    everything_everywhere_all_at_once.total_point_lkm = 0
    everything_everywhere_all_at_once.total_line_lkm = 0
    ...
    if everything_everywhere_all_at_once.output_lkm and not import_kml_instead:
        ...
        total_point_lkm = everything_everywhere_all_at_once.total_point_lkm
        total_line_lkm = everything_everywhere_all_at_once.total_line_lkm

        #create and open Excel file after process folder has navigated through all its files and branches
        try:
            output_lkm_excel_file_path = str(output_lkm_deploy_to_excel_Sharj.create_excel_file(folder_path=input_folder,total_point_lkm=total_point_lkm,total_line_lkm=total_line_lkm))
            try:
