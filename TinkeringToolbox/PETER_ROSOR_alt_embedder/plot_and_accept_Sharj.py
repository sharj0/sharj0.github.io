...
def run(waypoints, dsm, dem, surf_samples, grnd_samples):
    ...
    def show_canopy(ax,
        ...
        if not canopy.diff_arr:
            canopy.diff_arr, canopy.diff_x_coords, canopy.diff_y_coords = compute_difference(surf_arr, surf_x_coords,
                                                                                             surf_y_coords,
                                                                                             grnd_arr, grnd_x_coords,
                                                                                             grnd_y_coords)
    ...
    def set_altitude():
        try:
            ...
            new_waypoints = update_waypoints_altitude(waypoints, selected_indices, new_altitude)
            run(new_waypoints, dsm, dem, surf_samples, grnd_samples)
        except ValueError:
    ...
    # Accept Button
    accept_button = QPushButton("Change Boxed Waypoint Altitude", dialog)
    ...
    """Sharj"""
    bottom_bar_layout.addLayout(altitude_layout)
    bottom_bar_layout.addLayout(button_layout)
    """Sharj"""
    ...
    accept_button.clicked.connect(set_altitude)
