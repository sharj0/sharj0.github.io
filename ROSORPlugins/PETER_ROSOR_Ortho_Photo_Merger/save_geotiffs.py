...
def save_vrt_as_tiff(vrt_path, output_tiff_path, nodata_value=0, compress=True):
    """
    Save a VRT as a GeoTIFF with LZW compression and uint8 data type,
    and set a NoData value for each band. Prints progress in percentage.
    ...
    # Create the GeoTIFF with progress monitoring
    driver = gdal.GetDriverByName("GTiff")

    # Create a copy with a progress callback
    def progress_callback(complete, message, _userdata):
        percent = int(complete * 100)
        print(f"Processing: {percent}% completed", end="\r")

    tiff_ds = driver.CreateCopy(output_tiff_path, vrt_ds, options=options, callback=progress_callback)
    ...
    print("\nGeoTIFF saved successfully at:", output_tiff_path)
...
def apply_mask_to_rgb_rast(rgb_raster_path, mask_path, output_vrt_path, nodata_value=0):
    ...
    if mask_ds is None:
        raise RuntimeError("mask raster could not be opened.")

    if rgb_ds is None:
        raise RuntimeError("RGB raster could not be opened.")
...
if __name__ == '__main__':
    ...
    source_dir = r"R:\ORTHO_STUFF\Aurora_ortho_chunks\Final tiles"
    dest_dir = r"R:\ORTHO_STUFF\Aurora_ortho_chunks\re-upload"

    # List of file names to copy (extracted from the list you provided).
    files_to_copy = [
        "AF24.tiff", "AF25.tiff", "AF26.tiff", "AF27.tiff", "AF28.tiff",
        "AF32.tiff", "AF33.tiff", "AF34.tiff", "AF35.tiff", "AF36.tiff",
        "AF37.tiff", "AF38.tiff", "AF39.tiff", "AF40.tiff", "AF41.tiff",
        "AF42.tiff", "AF43.tiff", "AF44.tiff", "AG8.tiff", "AG24.tiff",
        "AH8.tiff", "AH24.tiff", "AI8.tiff", "AI9.tiff", "AI10.tiff",
        "AI11.tiff", "AI12.tiff", "AI13.tiff", "AI14.tiff", "AI15.tiff",
        "AI16.tiff", "AI18.tiff", "AI19.tiff", "AI20.tiff", "AI21.tiff",
        "AI22.tiff", "AI23.tiff", "AI24.tiff", "AJ9.tiff", "AK9.tiff",
        "AL9.tiff", "AM9.tiff", "AN8.tiff", "AO9.tiff", "AP8.tiff",
        "AQ8.tiff", "AS8.tiff", "T44.tiff", "U27.tiff", "V34.tiff",
        "V36.tiff", "V38.tiff", "Y22.tiff", "Y25.tiff", "Y30.tiff",
        "Y33.tiff", "Z21.tiff", "Z27.tiff"
    ]
    ...
    if False:
        ...
        for filename in files_to_copy:
            ...
            if os.path.exists(src_path):
                try:
                    # If the file exists in the destination, remove it.
                    if os.path.exists(dest_path):
                        os.remove(dest_path)
                        print(f"Deleted existing file in destination: {filename}")
    ...
    if True:
        #vrt_list = [r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\ns1.vrt",
        #            r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\ns2.vrt",
        #            r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\wn.vrt",
        #            r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\ws.vrt"]


        #vrt_list = [r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\_AI9-tripoint\AI9_n_masked.vrt",
        #            r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\_AI9-tripoint\AI9_s_masked.vrt",
        #            r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\_AI9-tripoint\AI9_w_masked.vrt"]

        vrt_list = [r"R:\ORTHO_STUFF\Aurora_ortho_chunks\Whole_area_actual_100_GSD\_Whole_area_actual_100_GSD_MERGED.VRT"]

        for vrt_path in vrt_list:
            output_tiff_path = vrt_path[:-4]+'_rgb.tiff'
            print(output_tiff_path)
            save_vrt_as_tiff(vrt_path, output_tiff_path, nodata_value=0)

        folder_list = [r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_NS_OVERLAP_N_1',
                       r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_NS_OVERLAP_N_2',
                       r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_NS_OVERLAP_S_1',
                       r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_NS_OVERLAP_S_2',
                       r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_WN_OVERLAP_N',
                       r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_WN_OVERLAP_W',
                       r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_WS_OVERLAP_S',
                       r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_WS_OVERLAP_W']

    r"R:\ORTHO_STUFF\Aurora_ortho_chunks\whole_left_side_100_GSD_v4\_whole_left_side_100_GSD_v4_MERGED.VRT"

    #print('\n\ntiff_list')
    #for folder in folder_list:
    #    print(folder)
    #    file_list = os.listdir(folder)
    #    tiff_list = [os.path.join(folder,file) for file in file_list if os.path.splitext(file)[1] == '.tiff']
    #    output_vrt_path = folder+'.vrt'
    #    print(tiff_list)
    #    merge_vrt_rasters(tiff_list, output_vrt_path)

    vrts_to_merge_list = [
        ['ns1_n_fixed_RGB.vrt', 'ns1_s_fixed_RGB.vrt'],
        ['ns2_n_fixed_RGB.vrt', 'ns2_s_fixed_RGB.vrt'],
        ['wn_n_fixed_RGB.vrt', 'wn_w_fixed_RGB.vrt'],
        ['ws_s_fixed_RGB.vrt', 'ws_w_fixed_RGB.vrt'],
    ]

    if False:
        vrts_to_merge_list = [[
            r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\_AI9-tripoint\AI9_n_masked.vrt",
            r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\_AI9-tripoint\AI9_s_masked.vrt",
            r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\_AI9-tripoint\AI9_w_masked.vrt",
        ]]

        for vrts_to_merge in vrts_to_merge_list:
            vrt_paths_to_merge = [os.path.join(r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\High res overlap tiles", vrt_name) for vrt_name in vrts_to_merge]
            output_vrt_name = vrts_to_merge[0][:3].strip("_")+".vrt"
            output_vrt_path = os.path.join(r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles",output_vrt_name)
            print(output_vrt_path)
            merge_vrt_rasters(vrt_paths_to_merge, output_vrt_path)

    vrt_and_mask_list = [
                    [r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_NS_OVERLAP_N_1.vrt', r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\High res overlap tiles\ns1_n_fixed.tiff"],
                    [r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_NS_OVERLAP_N_2.vrt', r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\High res overlap tiles\ns2_n_fixed.tiff"],
                    [r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_NS_OVERLAP_S_1.vrt', r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\High res overlap tiles\ns1_s_fixed.tiff"],
                    [r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_NS_OVERLAP_S_2.vrt', r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\High res overlap tiles\ns2_s_fixed.tiff"],
                    [r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_WN_OVERLAP_N.vrt',   r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\High res overlap tiles\wn_n_fixed.tiff"],
                    [r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_WN_OVERLAP_W.vrt',   r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\High res overlap tiles\wn_w_fixed.tiff"],
                    [r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_WS_OVERLAP_S.vrt',   r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\High res overlap tiles\ws_s_fixed.tiff"],
                    [r'R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\zz_WS_OVERLAP_W.vrt',   r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\High res overlap tiles\ws_w_fixed.tiff"],
                ]

    rgb_and_mask_list = [
        [r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\_AI9-tripoint\AI9_n.tiff",r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\_AI9-tripoint\6664_mask_n_fixed.tiff"],
        [r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\_AI9-tripoint\AI9_s.tiff",r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\_AI9-tripoint\6664_mask_s_fixed.tiff"],
        [r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\_AI9-tripoint\AI9_w.tiff",r"R:\ORTHO_STUFF\Aurora_ortho_chunks\tiles\_AI9-tripoint\6664_mask_w_fixed.tiff"]
    ]

    if False:
        for rgb_and_mask in rgb_and_mask_list:
            rgb_path, mask_path = rgb_and_mask[0], rgb_and_mask[1]
            output_vrt_path = rgb_path[:-5]+'_masked.vrt'
            print(output_vrt_path)
            apply_mask_to_rgb_rast(rgb_path, mask_path, output_vrt_path, nodata_value=0)

    if False:
        for vrt_and_mask in rgb_and_mask_list:
            vrt_path, mask_path = vrt_and_mask[0], vrt_and_mask[1]
            output_mask_path = mask_path[:-4] + "_fixed.tiff"

            print(f"Processing: {mask_path} → {output_mask_path}")

            # Open the mask dataset
            mask_ds = gdal.Open(mask_path, gdal.GA_ReadOnly)
            if mask_ds is None:
                print(f"Error: Could not open mask {mask_path}")
                continue
            ...
            if mask_array is None:
                print(f"Error: Could not read mask data from {mask_path}")
                continue

            # Convert all nonzero values to 1
            fixed_mask = np.where(mask_array > 0, 1, 0).astype(np.uint8)

            # Save the fixed mask as a new GeoTIFF
            driver = gdal.GetDriverByName("GTiff")
            out_ds = driver.Create(output_mask_path, mask_ds.RasterXSize, mask_ds.RasterYSize, 1, gdal.GDT_Byte)
            out_ds.SetGeoTransform(mask_ds.GetGeoTransform())
            out_ds.SetProjection(mask_ds.GetProjection())

            out_band = out_ds.GetRasterBand(1)
            out_band.WriteArray(fixed_mask)
            ...
            print(f"Fixed mask saved: {output_mask_path}")
    ...
    #vrt_path = r"R:\ORTHO_STUFF\Aurora_ortho_chunks\2,2mr_10_GSD\_2,2mr_10_GSD_MERGED.VRT"
    #output_tiff_path = r"R:\ORTHO_STUFF\Aurora_ortho_chunks\2,2mr_10_GSD\_2,2mr_10_GSD_FULL_TIFF_MERGED.TIFF"
    #save_vrt_as_tiff(vrt_path, output_tiff_path, nodata_value=0)
