...
def calculation_setup(csv_file):
    ...
    if 'Total_lkm' in mag_points_geodf.columns:
        everything_everywhere_all_at_one = Global_Singleton()
        try:
            everything_everywhere_all_at_one.total_line_lkm += mag_points_geodf['Total_lkm'][0]
        except:
            print("Attribute unavailable")
