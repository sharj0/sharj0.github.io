...
def split_csv_by_flightlines(csv_path, flightline_pairs, match_data_to_flt, gui):
    ...
    for pair in flightline_pairs:
        ...
        if not pair_df.empty:
            ...
            get_best_kml_for_mag_data, sub_samp_this_df, \
                Mag_Data, flights_list, match_thresh_percent = match_data_to_flt
            subsampled_df = sub_samp_this_df(pair_df)
            mag_data = Mag_Data(subsampled_df)
            best_kml = get_best_kml_for_mag_data(mag_data, flights_list, match_thresh_percent)
            ...
            if best_kml:
                ...
                if os.path.exists(new_export_file_path):
                    os.remove(new_export_file_path)
...
class FlightlineSplitterGUI(QWidget):
    def __init__(self, default_csv_path, match_data_to_flt):
        super().__init__()
    ...
    def initUI(self):
        ...
        self.setWindowTitle('Split flight by flightline')
    ...
    def run_splitter(self):
        ...
        self.show_message('Done splitting the flights. Consider replacing the original with this output')
...
def run_flightline_splitter_gui(no_match_csv_path, match_data_to_flt):
    app = QApplication.instance()  # Check if QApplication already exists
    if app is None:
        app = QApplication(sys.argv)
    gui_instance = FlightlineSplitterGUI(no_match_csv_path, match_data_to_flt)
    gui_instance.show()
    # Only call app.exec_() if there is no existing QApplication instance (not running in QGIS)
