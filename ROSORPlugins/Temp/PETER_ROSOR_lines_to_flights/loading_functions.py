...
from .Line_Class import Line_Class
...
def extract_line_obj_from_line_layer(flight_lines_layer, flight_lines_path):
    ...
    if all(strips):
        ...
        disp += f'\n✅ strips detected in input line layer'
    else:
    ...
    for layer_ind, feature in enumerate(flight_lines_layer.getFeatures()):
        ...
        for line in lines:
            start = Line_End_Class(line[0][0], line[0][1])
            end = Line_End_Class(line[1][0], line[1][1])
            line_obj = Line_Class(start, end,
                                  grid_fltlns[layer_ind],
                                  strips[layer_ind],
                                  ids[layer_ind],
                                  use_name,
                                  layer_ind, flight_lines_path)
